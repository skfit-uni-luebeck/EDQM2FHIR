...
class App:
    ...
    def generate_class_code_system(self) -> CodeSystem:
        ...
        output_cs.description = class_code_system_settings["description"].replace("<$date$>", self.generated_on)
    ...
    def create_code_system(self) -> CodeSystem:
        ...
        output_cs.copyright = self.fhir_metadata["copyright"]
        output_cs.publisher = self.fhir_metadata["publisher"]
        output_cs.description = cs_settings["description"].replace("<$date$>", self.generated_on)
    ...
    def create_value_sets(self) -> List[ValueSet]:
        return []
    ...
    def __cs_generate_concepts(self, all_terms: Dict):
        ...
        for concept in all_terms["content"]:
            ...
            # trim added due to validation using ontoserver.csiro.au.de/r4
    ...
    def __generate_div(self, resource: CodeSystem | ValueSet) -> Narrative:
        ...
        div = div_template.replace("<$title$>", title)
        div = div.replace("<$canonical$", canonical)
        div = div.replace("<$description$>", description)
        return Narrative(**{
            "status": "generated",
            "div": div
        })
